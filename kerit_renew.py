...
def do_renew(sb, ip_info=None, email=None):
    ...
    for attempt in range(need):
        ...
        for _ in range(15):
            try:
                if sb.is_element_visible('#adBanner') or sb.is_element_visible('[onclick="openAdLink()"]'):
                    ad_visible = True
                    break
            except Exception:
                pass
        ...
        print("⏳ 等待赞助商新窗口...")
        try:
            sb.wait_for_new_window(timeout=10)
            sb.switch_to_newest_window()
            print("🌐 已切换到赞助商页面")
            time.sleep(5)  # 模拟访问
            sb.driver.close()  # 关闭新窗口
            sb.switch_to_default_window()  # 回到原窗口
            time.sleep(2)
        except Exception as e:
            print(f"⚠️ 处理新窗口异常: {e}")
            # 如果没出现新窗口，可能是在原窗口内打开的，强制回到默认窗口并刷新
            try:
